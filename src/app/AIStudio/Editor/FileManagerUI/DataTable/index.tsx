...
export function DataTable<T>(props: {
    ...
    /**
     * The function is called when a file is dragged over the table. It's emitted
     * 
     */
    onFileDragOver?: () => void,
    data: T[],
    ...
    return <div class="flex gap-2.5 flex-col min-w-[400px]" onDrop={(ev) => {
        ev.preventDefault();
            ...
            <For each={props.headers}>{(header, ind) => {
                return <DataTableHeader name={header.name} sortState={currentlySelectedHeader() === ind() ? sortState() : HeaderSortState.NotSelected} noResize={header.noResize} onClick={() => {
                    batch(() => {
                        ...
                        setData(header.sort(sortState(), props.data));
                    });
                }} onResize={(width) => {
        ...
        <DataRowList data={data()} resizeInfo={resizeData} resolveData={props.resolveData} />
    </div>
}
